...
/// let camera = Camera::new(Coord::default(), Angle3D::default(), 90.0);
    ...
    fn generate_shape(&self, shape: &mut Shape) {
        ...
        while a < 2.0 * PI {
            ...
            while b < 2.0 * PI {
                ...
                let point_x: f64 = (self.radius + (self.thickness * cos_b)) * cos_a;
                let point_y: f64 = (self.radius + (self.thickness * cos_b)) * sin_a;
                let point_z: f64 = self.thickness * sin_b;

                let point_coord = Coord::new(point_x, point_y, point_z);

                let normal_x: f64 = sin_b;
                let normal_y: f64 = cos_b * cos_a;
                let normal_z: f64 = cos_b * sin_a;
                ...
                b = b + self.angle_iter.1;
            }
            a = a + self.angle_iter.0;
        }
    }
}
